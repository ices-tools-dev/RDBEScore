...
# how this example is placed in RDBES
	...
	# 1 child SD row
	# 184 child rows in VS (the 184 clutches/psus observed)
		# each associated to its cluster (clutch)
		# VSnumberTotal is not known (see page 59)
		# VSnumberSampled is 184
  #368 child rows in the FT table (the 368 eggs/ssus measured)
      #each associated to its vessel (clutch)
      #FTnumbersampled is 2
      #FTnumberTotal is csize
	...
	# load textbook data
		...
		df<-coots
		target_var<-"volume"
		targetMultiplyer<- 100000000L

	# name your project (will be used in filenames for CS, SL and VD)
		project_name_outputs <- "WGRDBES-EST_TEST_1_Pckg_SDAResources_coots_multistage_H1"
	...
	# select a year for upload
		...
		DEstratumName <- "Pckg_SDAResources_coots_multistage_H1"
...
#========Outline of Hierarchy 1================
	...
	# Length
		...
		df_vessel <- df[!duplicated(df[c("clutch")]),]
		# adds VSid to df
		...
		VS_df <- data.frame(
		  ...
		  VSunitName = paste0("clutch_",df_vessel$clutch),#M
		...
		makeChildTbl <- function(parent, data, tbl="FT", by = "dname"){
		  ...
		  FT_df <- data.frame(
		    ...
		    FTnumberTotal= as.vector(df$csize), #[DV,O] - int
		...
		FT_df <- makeChildTbl(df_vessel, df, by="clutch")
		...
		SA_df<-data.frame(
		  ...
		  SAtotalWeightMeasured = ifelse(!is.na(df[[target_var]]),df[[target_var]]*targetMultiplyer,""), #
		  SAsampleWeightMeasured = ifelse(!is.na(df[[target_var]]),df[[target_var]]*targetMultiplyer,""), #
