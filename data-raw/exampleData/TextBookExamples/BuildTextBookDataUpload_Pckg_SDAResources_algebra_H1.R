#======Prepares textbook data SDAResources::algebra as H1 upload file===========
...
# ?algebra
	...
	# interpretation 
		...
		# clusters are classes (class)
		# clusters (psu) are unequal sized (Mi). In each cluster, all students are selected (ssus, nrows).
	# total number of psus is known (187) 
	# target variable is score

# how this example is placed in RDBES
	# 1 DE row with DEstratumName == "Pckg_SDAResources_algebra_H1"
	# 1 child SD row
	# 299 child rows in VS (the 299 students observed)
		# each associated to its cluster (class)
		# VSnumberTotalClusters is 187 (see ?algebra)
		# VSnumberSampledClusters is 12 (see ?algebra)
		# VSnumberTotal is Mi
	# tables FT, FO, SS are just 1:1 links to the final data (in SA)
	# in SA each score is a SAsampleWeightMeasured
	...
	# name your project (will be used in filenames for CS, SL and VD)
		project_name_outputs <- "WGRDBES-EST_TEST_1_Pckg_SDAResources_algebra_H1"
...
# adds VSid to dataset
	...
	VS_base<-as.data.table(dataset)[,list(N=Mi,n=.N),class]
...
# clusters
VS_df$VSnumberTotalClusters<-187 
VS_df$VSnumberSampledClusters<-12
